...
class Decoding_model(keras.Model):
    ...
    def call(self,inputs,labels): 
        ...
        fer,ber,undected_count,indices = self.get_eval(soft_output_list,iteration,labels)
        buffer = self.collect_failed_output_selective(soft_output_list,labels,indices)
        return fer,ber,undected_count,buffer
    def calculation_pseudocrossentropy(self,soft_output_list,estimated_first,estimated_last):
         #cross entroy
        estimated_first = tf.cast(estimated_first,tf.float32)
        estimated_last = tf.cast(estimated_last,tf.float32)
        CE_loss1 = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=-soft_output_list[0], labels=estimated_first)) 
        CE_loss2 = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=-soft_output_list[0], labels=estimated_last)) 
        CE_loss3 = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=-soft_output_list[-1], labels=estimated_first)) 
        CE_loss4 = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=-soft_output_list[-1], labels=estimated_last)) 
        return [CE_loss1,CE_loss2,CE_loss3,CE_loss4]
    def print_undeteced(self,soft_output_list,iteration,diff_set,correct_set):           
            print('undetected errors:',sorted(diff_set))
            print('corrected errors:',sorted(correct_set))
            #print('A-B:',set(index1)-set(index))
            for index in sorted(diff_set):
                estimated_label_first = tf.where(soft_output_list[0][index]>0,0,1)
                estimated_label_last = tf.where(soft_output_list[iteration][index]>0,0,1)
                indexed_outputs = [soft_output_list[i][index] for i in range(iteration+1)]
                undetected_CE = self.calculation_pseudocrossentropy\
                    (indexed_outputs, estimated_label_first, estimated_label_last)
                       
                tf.print(index,undetected_CE,tf.reduce_sum((estimated_label_first+estimated_label_last)%2))
            for index in sorted(correct_set):
                estimated_label_first = tf.where(soft_output_list[0][index]>0,0,1)
                estimated_label_last = tf.where(soft_output_list[iteration][index]>0,0,1)
                indexed_outputs = [soft_output_list[i][index] for i in range(iteration+1)]
    
                correct_CE = self.calculation_pseudocrossentropy\
                    (indexed_outputs, estimated_label_first,estimated_label_last)
                tf.print(index,correct_CE,tf.reduce_sum((estimated_label_first+estimated_label_last)%2))
