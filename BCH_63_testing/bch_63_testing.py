...
sys.argv = "python 0.0 3.5 8 100 100 4 BCH_63_45_3_strip.alist NMS-1".split()
...
GL.set_map('decoding_threshold',3000)
GL.set_map('regular_matrix',False)
GL.set_map('generate_extended_parity_check_matrix',False)
GL.set_map('reduction_iteration',3)    
...
GL.set_map('print_interval',20)
GL.set_map('record_interval',100)
GL.set_map('penalty_weight',1)
GL.set_map('num_shifts',3)
...
if decoder_type != 'SPA':
    restore_ckpts_dir = '../BCH_'+str(n_dims)+'_training/ckpts/'+decoder_type+'/'+str(n_iteration)+'th'+'/'
...
for SNR in SNRs:
    ...
    sigma = np.sqrt(1. / (2 * (float(code.k)/float(code.check_matrix_col)) * 10**(SNR/10)))
    GL.set_map('noise_standard_variance',sigma)
    dataset_test = Reading.data_handler(code.check_matrix_col,iput_file,unit_batch_size)
    ...
    FER_list.append((snr,round(average_test_fer1.numpy(),5)))
