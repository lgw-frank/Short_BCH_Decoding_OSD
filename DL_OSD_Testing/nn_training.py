...
def identify_mrb(order_H_list):
    ...
    for i in range(order_H_list.shape[0]):
        ...
        jump_point = (code.check_matrix_column-code.k)-2*GL.get_map('delimiter')
...
def evaluate_MRB_bit(inputs,labels):
    inputs_abs = tf.abs(inputs)
    code = GL.get_map('code_parameters')
    order_index = tf.argsort(inputs_abs,axis=-1,direction='DESCENDING')
...
def Training_NN1(selected_ds,restore_info,indicator_list,prefix_list,DIA=False):
...
def Training_NN(selected_ds,restore_info,indicator_list,prefix_list,DIA=False):
    ...
    for i in range(num_counter):
        ...
        dic_sum = dic_union(dic_sum,cmp_results)
        #query pattern distribution
        pattern_cnt,swap_len_list = generate_pattern_dist(updated_inputs,labels,pattern_cnt) 
        swap_list += swap_len_list
        if (i+1)%100 == 0:
            print(dic_sum) 
            #print(pattern_cnt) 
...
def generate_pattern_dist(inputs,labels,pattern_cnt):
    Updated_MRB_list,swap_len_list = stat_pro_osd(inputs,labels)    
    pattern_cnt = evaluate_MRB_pattern(inputs,labels,Updated_MRB_list,swap_len_list,pattern_cnt) 
    return pattern_cnt,swap_len_list


def evaluate_MRB_pattern(inputs,labels, updated_mrb_list,swap_len_list,pattern_cnt):
    inputs_abs = tf.abs(inputs)
    ...
    error_sum = np.sum(diff_matrix,axis=-1)
    string_list = [str(error_sum[i]) for i in range(inputs.shape[0])]

    def inline_iteration(i):
        delimiter1 = fixed_gap
        delimiter2 = 2*fixed_gap+delimiter1
        LR_part = np.sum(diff_matrix[i][:delimiter1])
        MR_part = np.sum(diff_matrix[i][delimiter1:delimiter2])
        HR_part = np.sum(diff_matrix[i][delimiter2:])
        cmp_result = [LR_part,MR_part,HR_part]
        return cmp_result
    string_list = [str(inline_iteration(i)) for i in range(inputs.shape[0])]
